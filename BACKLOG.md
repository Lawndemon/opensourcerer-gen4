...
**Last updated:** 2026-07-21 (ICS forms Phase 3 — curated field maps + per-form AI auto-fill live for 201/202/208/214/215A; forms are the purchase-decision feature per Dave)
...
**WITHDRAWN by Dave 2026-07-21, hours after filing:** with the PDF-first tabs, "we can now work directly within the PDF" — the browser viewer's interactive AcroForm fields make a bespoke add-entry affordance unnecessary. _Standing caveat (assistant): in-browser PDF edits are LOCAL ONLY — they never persist to the incident record, other roles' views, regenerations, or the audit trail. The "Edit fields" → Save path remains the only persisted-edit surface (`form_content_edited` audit event). Fine for print/save-a-copy workflows; revisit only if the SME expects in-PDF edits to become part of the official record (that would mean a save-back path — e.g. re-uploading the edited PDF and parsing fields out — a materially bigger feature)._
