...
**Last updated:** 2026-07-21 (the forms day, rounds 1–6: curated field maps + AI auto-fill for 6 forms; PDF-first tabs → edit-in-PDF with Cosmos harvest via PdfFormViewer; concurrent-edit conflict detection; locked-form server gate; stale-Easy-Auth-token 403 mystery solved)
...
### 2026-07-21 — ICS forms Phase 3: curated field maps + per-form AI auto-fill (the purchase-decision feature)

**Context (Dave, 2026-07-21):** the rest of the app demos well; the FORMS are what get buyers over the line — and they were "not working": every PDF-backed form arrived blank (the Phase-3 auto-fill was never built; `extract_forms` deliberately discarded LLM content for PDF-backed forms) and the editor showed raw AcroForm names ("Item 1", `Text57`) wherever the /TU scrape had no label. Decision: do NOT rebuild forms from the ground up — keep the AcroForm pipeline (the export IS the official ICS Canada PDF) and fix fill + labels via **curated field maps**, one artifact per form serving both the editor labels and the AI fill prompt.

**New artifact — `incidents/ics_pdf_templates/field_curation.json`.** Per form, per AcroForm field: `label` (human label, overrides the scraped /TU), `guidance` (what belongs here; included verbatim in the fill prompt), `ai_fill` (should the LLM attempt it — signatures/attachment-checkboxes/page-numbers stay false). Merged onto the machine-generated `schemas.json` inside `pdf_filler.load_schemas()`, so `/api/ics-forms/schemas` (the editor) and the fill prompt read ONE merged view. **Zero frontend changes needed** — better data in, better editor out.

**Curated + AI-fillable (5 forms, 100% label coverage, ~250 fillable fields):** ICS-202 (15), ICS-208 (12), ICS-215A (51 — the demo star: 14 hazard-analysis rows of Incident Area / Hazards / Mitigations that map straight from scene conditions), ICS-201 (92 — situation summary, objectives, 16-row time/action log, current org, resource table rows 1–10), ICS-214 (80 — 15-row activity log from the transcript timeline; page-2 continuation rows labeled but ai_fill=false to keep the prompt lean). Mystery fields decoded by **marker-filling the PDFs and reading them visually** (Text15 = 208's Time Prepared; 215A's Text1–6 = date/time prepared + op period; 214's Text37–70 = the page-2 continuation grid in scrambled pairs; 201's parent nodes `4`,`5`,`7`… are internal — labeled "leave blank").

**Fill pipeline (`approaches/extract_forms.py`):** after the canonical form list is built, `_fill_curated_pdf_forms` runs ONE fill call per distinct `form_id_key` that has curated fillable fields (deduped across roles, `asyncio.gather`), new prompt `prompts/extraction/fill_form_fields.md` (exact-fieldName JSON out; omit-if-ungrounded; never fabricate identifiers — names/units/frequencies are evidence-only; checkboxes emit "Yes"). Results merge over any seed values (the old ICS-201 7-field typed→AcroForm bridge stays as seed). A failed per-form fill logs a warning and leaves that form blank — never fails the extraction. **ICS-214 role stamping:** one shared fill, then each role's copy gets `5. ICS POSITION` stamped with its owner's title via new `ROLE_TITLES` map in `form_templates.py` (curation marks that field ai_fill=false so the LLM never competes with the stamp).

**`pdf_filler.py` hardening:** checkbox ("button") fields now fill correctly — truthy spellings (yes/true/checked/x/on/1) → `/Yes`, else `/Off` (all 11 templates use `/Yes` as on-state); `sanitize_field_text` transliterates typographic characters (em-dash, curly quotes, ellipsis…) that the official forms' font encodings can't represent (pypdf corruption warning found during smoke test). Note **Form-208.pdf is AES-encrypted** (blank user password) — `cryptography` is already in requirements, but any local venv poking at the PDFs needs it too.

**Still to do (next batch):** 204, 205, 211 curation (labels are 73–94% already; mostly guidance + ai_fill flags). **Deferred (worst effort-to-value):** ICS-207 org chart (114 unlabeled position boxes) and ICS-215 worksheet (370-field grid) — downloadable-but-manual. Non-PDF forms (209, 213, 206, 218, OF-288, 226, 219) unchanged: placeholder content; ground-up build remains the fallback for those only.

**Round 2 (same day, post Dave's runtime test — "they look much better now!", samples saved in `referencedocs/sample_test_ICS_forms/`):**

- **PDF-first form tabs (Dave's call: "the PDF is the thing").** Dave's verdict on round 1: forms look really good *as PDFs*; the app's field-list input view is the problem (ICS-201/214 "repeated information" = the editor rendering 4× Prepared-By + N table-row textareas, not a fill bug — the PDFs were clean). Shipped: `FormTabStrip` now renders the **populated official PDF embedded (iframe)** as the default overlay for any `form_fields` form on a persisted incident — FO kiosk and support views alike. Toolbar: **Open full screen** (new tab) + **Edit fields** (editable views only — flips to the old field editor; Save lands back on the PDF; "View PDF" cancels out). Cache-busted via `lastUpdated` in iframe key + query param. Kiosk now passes `incidentId` (persisted only); ephemeral incidents keep the old summary card. CloseoutAdmin's editor untouched. **Caveat flagged: iframe PDF rendering is fine on desktop (demo surface) but iPadOS Safari inline-PDF support is historically poor — "Open full screen" is the fallback; test on the real kiosk iPad before a live demo.**
- **ICS-203 curated** (117/117 fields labeled, 42 AI-fillable): command staff + section-chief name boxes fill only when names are stated in evidence; 6. Agency Representatives rows fill from mutual-aid mentions; branch/division/air-ops grids labeled but human-filled. Was "mostly blank + wonky all caps" in Dave's test — blankness is inherent until names/org data exist in evidence (see KB-enrichment item below).
- **Fill-prompt rule 7 added** (table cells): Row-N fields are narrow grid cells — one short item each, spread lists across rows. Fixes the "Engine 1 on scene; remai…" truncation Dave's ICS-201 sample caught in the resources table.

### 2026-07-21 (Round 3) — The intermittent-403 mystery solved: stale Easy Auth tokens on the incidents API

**Symptom (Dave, same day):** `GET .../support-recommendations?role=safety-officer` → HTTP 403 on the Safety Officer page, appearing ~2h into a test session. **Root cause — and it's the same bug as the May-27 "attest 403s" that got a diagnostic logger and was never pinned down:** the chat API always attached a fresh bearer token via `getToken()` → `getAppServicesToken()`, which transparently calls `.auth/refresh` when the Easy Auth token expires. The **incidents API client attached no Authorization header at all** — it relied on Container Apps Easy Auth's injected `x-ms-token-aad-access-token`, which goes stale after ~1h because nothing ever refreshed the token store. Stale token → backend OBO exchange fails → `AuthError` → `@authenticated` aborts 403. Intermittent because it only bit sessions older than the token lifetime.

**Fix (frontend-only):** new `getBearerAuthHeaders()` exported from `authConfig.ts` (wraps the existing `getAppServicesToken()` refresh machinery; returns `{}` outside app-services auth). Every fetch in `api/incidents.ts` (postJson / jsonRequest / validateIAP / getIcsFormSchemas) now attaches it. Two effects: the backend prefers the fresh Authorization header over the stale injected one, and the `.auth/refresh` side effect keeps the cookie token store fresh — which also keeps the **PDF iframe** (cookie-authenticated, can't send headers) alive. Since the support view polls every 10s, tokens now stay perpetually fresh. The May-27 diagnostic logger in `decorators.py` can come out once this soak-tests clean. **Same-round confirmation:** Dave's "forbidden access on first ICS-214 open on the FO portal, fine after opening it on the IC portal" was this exact bug seen from the iframe side (the token store is per-login-session, so the IC-portal open refreshed it for both tabs) — pre-dated the fix deploy.

**PDF disposition fix (same round):** the form-PDF endpoint sent `Content-Disposition: attachment`, so the PDF-first iframe triggered a download → Adobe instead of rendering embedded (Dave: "opens in Adobe... I would really like it embedded"). Now `inline` by default (browser's built-in viewer renders inside the iframe); `?download=1` opts back into attachment for callers that want a file.

### 2026-07-21 (Round 4) — Edit-in-PDF with Cosmos harvest: PdfFormViewer replaces both the iframe and the field editors

**Dave's requirement, following the local-only caveat on the withdrawn add-entry item:** "I like the UI that comes with editing the PDFs directly but we need to harvest changes from them so that the changes are shared across roles and logged in cosmos" — plus the same for the Closeout Reporting editors. The browser's native PDF viewer is a sealed plugin (typed values are unreadable by the page — the reason the iframe approach couldn't harvest), so the rendering engine changed to **PDF.js** (`pdfjs-dist@6`, self-hosted, vite-bundled worker).

**New `components/PdfFormViewer.tsx`** — the one component now used by BOTH the form tabs and CloseoutAdmin:
- Fetches the filled PDF with bearer auth (no more cookie-dependent iframe), renders each page to canvas with `annotationMode: ENABLE_FORMS` (PDF.js deliberately skips painting field contents in that mode — official artwork only), then overlays absolutely-positioned HTML inputs (textarea / checkbox) at each AcroForm widget rectangle, seeded from the incident's authoritative field map. Deliberately NOT PDF.js's AnnotationLayer forms (API churn, no control) — the overlay is ours, so harvest is a trivial controlled-React read.
- Editable mode tints fields, tracks dirty state, and **Save** merges edits over `content.fields` → existing `saveFormContent` endpoint → Cosmos + `form_content_edited` audit + cross-role sharing via the standing polls. Read-only mode (FO kiosk voice-and-button doctrine; locked incidents/forms) renders untinted and disabled.
- Multi-widget same-name fields (e.g. ICS-214's shared Time From on both pages) bind to one value; checkboxes round-trip via the same truthy convention as `pdf_filler.py`.

**Wiring:** `FormTabStrip` — iframe replaced by PdfFormViewer; the old inline field-list editor and its Edit-fields flip are **deleted** (editing happens on the form itself now). `CloseoutAdmin` — `FormFieldsFields` (the textarea-wall editor) **deleted**; PDF-backed forms render PdfFormViewer with its own Save; ICS-201-typed + placeholder forms keep the classic editors; Download PDF uses `?download=1`. This resolves the round-3 caveat: in-PDF edits are now the persisted, shared, audited path — the "(+) add entry" withdrawal stands with no strings attached.

**Known limits (recorded, not blocking):** signature/choice widgets not overlaid (text + checkbox only — covers all 11 templates' fillable content); the FO kiosk stays read-only pending SME word otherwise.

**Round 6 (same day) — locked-form edits gated server-side (Dave's governance question).** Dave asked whether all portals/forms lock down with only the IC able to edit "once he/she has closed the incident." Audit answer: the terminal Close-out & Lock is already absolute for everyone (423 via the `replace_incident` guard — stricter than asked; Reopen stays the backlogged escape hatch), and post-Close-Incident the form tabs honored `status == "locked"` — but **only in the UI**; `save_form_content` never checked form status server-side. Closed: locked forms now reject saves with 403 unless the actor is `incident-commander` or `site-administrator` (the cleanup-authorized pair, matching `/lock` authorization). Net rule, now server-enforced end to end: **open → owning role edits; form locked (Loss Stop / Close Incident) → IC + Site-Admin only; event locked → no one (until Reopen exists).**

**Round 5 (same day) — concurrent-edit conflict detection (optimistic locking; Dave chose this over presence locks).** Dave asked whether a second editor gets a graceful "the Safety Officer is currently editing this form" — answer was no (last-write-wins, silent). Options presented: (1) optimistic conflict detection vs (2) true presence leases; Dave picked (1) ("sounds fine for now"), noting surprise that locks are the harder one — recorded rationale: a lock is a *promise* needing heartbeats/expiry/stale-lock recovery and honest low-latency presence (our 10s polls would lie); a stamp check keeps no promises. Implementation: `SaveFormContentRequest.expected_last_updated` (optional — legacy callers keep LWW); `save_form_content` raises new `FormEditConflictError` on stale stamp (scans event log backwards for the last `form_content_edited`/`form_generated` toucher) → endpoint 409 `{conflict, lastUpdatedBy}`; `PdfFormViewer` sends its base stamp, and on 409 **rebases onto the newer save without losing the user's typing** (base fields swap underneath, local edits stay on top) with the message "This form was updated by the {Role} while you were editing — their changes are now loaded underneath yours; review and Save again." Presence locking (option 2) deliberately NOT built — revisit only if the SME confirms simultaneous multi-editor workflows matter.

### Form KB-enrichment — fill form fields from the knowledgebase, not just the ERP session (Dave, 2026-07-21)

Dave's observation from testing: some form content **cannot come from the incident session** — references to document standards, org charts/rosters, agency contact details. The fill pipeline currently sees only scene + transcript. Proposal shape: a retrieval step per form (or per field-group) that queries the KB/RAG for the relevant reference material (org roster → ICS-203 name boxes; standards references → safety-plan citations), merged into the fill prompt as an additional evidence block. Ties directly into the tiered retrieval cascade (Client > Region > Federal > Domain) and becomes materially better post-[[Planned engine migration: OpenAI → Anthropic]]. Design questions to settle before building: which KB docs carry org data per tenant, and how field-curation marks a field as KB-sourced vs session-sourced (likely a `kb_hint` key in field_curation.json).

### ~~Form editor "(+) add entry" dynamic rows for support roles (Dave, 2026-07-21)~~ — WITHDRAWN same day
...
### 2026-06-11 — Scene setup rework: type/narrate option + voice dictation; inject popup text/voice-only
...
**Pre-ToC role-control restrictions (Dave, 2026-06-11, second QoL):** until the IC takes command, only the FO-bypass roles (**Safety Officer + Ops Section Chief**) can be taken control of by a human — every other support role's Take Control unlocks at Transfer of Command (UI shows "Take Control available after Transfer of Command" instead of the button). And pre-ToC, even SO/OSC get only **own + send to FO** — `send_to_ic` now requires `commandTransferredAt != null` (there's no human IC to route to). Changed in BOTH twin routing modules (`recommendationRouting.ts`: `canTakeControl` + `routingActionsFor`; `incidents/routing.py`: `can_take_control`/`assert_can_take_control` + `routing_actions_for`) and enforced server-side in `take_control` (403). The "→ IC" button auto-hides pre-ToC via the existing `routingActions.includes("send_to_ic")` wiring; `assert_publish_target_allowed` rejects `target="ic"` pre-ToC automatically. Verified: `ast.parse` clean ×3, full-project `tsc --noEmit` 0 errors. **Mount-staleness note:** the recurring Windows→Linux truncation hit `routing.py`, `recommendationRouting.ts`, `IncidentSupportView.tsx`, and `RecommendationRow.tsx` this session — all four heredoc-resynced per standing practice.
