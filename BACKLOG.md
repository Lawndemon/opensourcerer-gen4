...
### Form editor "(+) add entry" dynamic rows for support roles (Dave, 2026-07-21)

For multi-row forms (201 actions/resources, 214 activity log), support roles (NOT the FO) should append entries without hunting for the next empty pre-rendered row: an "(+) add entry" affordance that finds the first blank row of that table and focuses it (the official PDFs have fixed row counts, so "add" = fill-next-blank, with a "form full" state when rows run out). Lives in the field editor / CloseoutAdmin; the PDF-first view stays read-only. Also pairs with ICS-214 continuation-page rows (currently ai_fill=false, rows 16–45 labeled and ready).
